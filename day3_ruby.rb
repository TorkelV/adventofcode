...
lines = input.map{ |e| 
  ...
  obj = {"fy" => [lasty,y].min, "fx" => [lastx,x].min, "tx" => [x,lastx].max, "ty" => [y,lasty].max, "type" => type(e["dir"]), "steps" => steps, "fromx" => lastx, "fromy" => lasty}
  steps += e["steps"]
  obj
}
...
input2.each{ |e|
  ...
  c = {"fy" => [lasty,y].min, "fx" => [lastx,x].min, "tx" => [x,lastx].max, "ty" => [y,lasty].max, "type" => type(e["dir"]), "steps" => steps, "fromx" => lastx, "fromy" => lasty}
  lines.each{ |l|
    if c["type"] == "V" && l["type"]=="H" && l["fx"] <= c["fx"]  && c["fx"] <= l["tx"] && c["ty"] >= l["fy"] && c["fy"] <= l["fy"] 
      distance = c["fx"].abs + l["fy"].abs;
      fSteps = c["steps"] + (c["fromy"]-l["fromy"]).abs + l["steps"] + (c["fromx"]-l["fromx"]).abs
      if distance > 0
        if fSteps<minSteps
          minSteps = fSteps
        end
    ...
    if c["type"] == "H" && l["type"]=="V" && l["fy"] <= c["fy"]  && c["fy"] <= l["ty"] && c["tx"] >= l["fx"] && c["fx"] <= l["fx"]
      distance = c["fy"].abs + l["fx"].abs
      fSteps = c["steps"] + (c["fromx"]-l["fromx"]).abs + l["steps"] + (c["fromy"]-l["fromy"]).abs
      if distance > 0
        if fSteps<minSteps
          minSteps = fSteps
        end
...
print("Steps: " + minSteps.to_s)
print("\n")
print("Distance: " + minDistance.to_s)
